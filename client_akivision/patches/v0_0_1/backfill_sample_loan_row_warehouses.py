...
def execute():
    """Backfill per-row warehouse fields from parent header for existing sample loan documents."""
...
def backfill_sample_loan_out():
    ...
    for doc in frappe.get_all("Sample Loan Out", fields=["name", "source_warehouse", "loan_warehouse"]):
        frappe.db.sql(
            """
            UPDATE `tabSample Loan Out Item`
            SET source_warehouse = %s, loan_warehouse = %s
...
def backfill_sample_loan_out_return():
    ...
    for doc in frappe.get_all("Sample Loan Out Return", fields=["name", "sample_loan_out"]):
        ...
        for row in frappe.get_all(
            ...
            # Prefer per-row warehouse from parent loan if available
            parent_row = frappe.db.get_value(
                "Sample Loan Out Item",
                {"parent": doc.sample_loan_out, "parenttype": "Sample Loan Out", "serial_no": row.serial_no},
                ["source_warehouse", "loan_warehouse"],
                as_dict=1,
            )
            if parent_row:
            ...
            frappe.db.set_value(
                "Sample Loan Out Return Item",
                row.name,
                {"source_warehouse": source_wh, "loan_warehouse": loan_wh},
            )
