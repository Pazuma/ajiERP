...
function cf_item_datatable(frm, key, container, columns, data, no_data_message, tree_view) {
...
function get_bom_where_used_columns() {
    return [
        ...
        { id: "finished_item_name", name: __("成品名称"), width: 150, editable: false, focusable: false },
        {
        ...
        { id: "is_default_text", name: __("默认"), width: 60, editable: false, focusable: false, align: "center" },
        { id: "is_active_text", name: __("启用"), width: 60, editable: false, focusable: false, align: "center" },
        { id: "company", name: __("公司"), width: 90, editable: false, focusable: false },
    ];
}
...
function render_bom_where_used_html(frm) {
    ...
    const rows = (data.rows || []).map((row) => ({
        ...
        is_default_text: row.is_default ? __("是") : __("否"),
        is_active_text: row.is_active ? __("启用") : __("停用"),
        company: row.company,
    }));

    $wrapper.html(
        cf_item_block_html(title, `<div class="cf-item-datatable cf-bom-where-used-datatable"></div>`)
    );

    const container = $wrapper.find(".cf-bom-where-used-datatable").get(0);
    if (!container) {
        return;
    }
    ...
    cf_item_datatable(
        ...
        "_cf_bom_dt",
        container,
        get_bom_where_used_columns(),
        rows,
        __("该物料未被任何已提交的 BOM 使用")
    );
...
function get_tier_pricing_columns() {
    return [
        ...
        { id: "side_text", name: __("买/卖"), width: 90, editable: false, focusable: false, align: "center" },
        {
            ...
            align: "center",
            format: (value, row, column, data) => {
                const disabled = data && data.is_disabled;
                return `<span class="${disabled ? "text-muted" : ""}">${cf_item_escape(value)}</span>`;
            },
        },
    ];
}
